...
综上，写一个map的循环时，特别时在写底层框架时，一个简单的for循环变的隐患无数。要根据需求，容器结构，容器的大小，性能要求决定合适的方式，比较安全的方式我暂时也就总结以上四种。最后，再考虑下其它容器的情况？list、vector、unorderd_map？有更好方案的可以留言或邮件：397157852@qq.com。
